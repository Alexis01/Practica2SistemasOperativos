// Práctica 2 de SO - Valentín Blanco Gómez y Antonio Calvo Morata
...
int myExport(MiSistemaDeFicheros* miSistemaDeFicheros, char* nombreArchivoInterno, char* nombreArchivoExterno) {
	uint i = 0;
	const uint tamBloqueBytes = miSistemaDeFicheros->superBloque.tamBloque;
	uint idxArchivo;
	int handle;
	EstructuraNodoI *temp;
	char sobreEscribir;
	uint bytesUltimos;
	uint bloquesIteracion;
	char buffer[tamBloqueBytes];

	/// Buscamos el archivo nombreArchivoInterno en miSistemaDeFicheros
	if ((idxArchivo = buscaPosDirectorio(miSistemaDeFicheros, nombreArchivoInterno)) == -1) {
		fprintf(stderr, "Error, un fichero con nombre %s no existe en el directorio\n", nombreArchivoInterno);
		return 0;
	}

	/// Si ya existe el archivo nombreArchivoExterno en linux preguntamos si sobreescribir
	if ((handle = open(nombreArchivoExterno, O_CREAT | O_EXCL | O_WRONLY, S_IRUSR | S_IWUSR)) == -1) {
		printf("El archivo %s ya existe en el directorio de exportación, desea reemplazarlo? Sí(s)/No(n)\n", nombreArchivoExterno);
		scanf("%c", &sobreEscribir);

		if (sobreEscribir == 'n')
			return 0;

		handle = creat(nombreArchivoExterno, S_IWUSR);
	}

	/// Copiamos bloque a bloque del archivo interno al externo
	temp = miSistemaDeFicheros->nodosI[miSistemaDeFicheros->directorio.archivos[idxArchivo].idxNodoI];
	bloquesIteracion = temp->numBloques - 1;
	bytesUltimos = temp->tamArchivo - bloquesIteracion*tamBloqueBytes;

	for (i = 0; i < bloquesIteracion; ++i) {
		if (lseek(miSistemaDeFicheros->discoVirtual, temp->idxBloques[i]*tamBloqueBytes, SEEK_SET) == (off_t) -1) {
			fprintf(stderr, "Falló lseek en myExport");
			return 0;
		}

		if (read(miSistemaDeFicheros->discoVirtual, &buffer, tamBloqueBytes) == -1) {
			fprintf(stderr, "Falló read en myExport");
			return 0;
		}

		if (write(handle, &buffer, tamBloqueBytes) == -1) {
			fprintf(stderr, "Falló write en myExport");
			return 0;
		}
	}

    if (lseek(miSistemaDeFicheros->discoVirtual, temp->idxBloques[i]*tamBloqueBytes, SEEK_SET) == (off_t) -1) {
		fprintf(stderr, "Falló lseek en myExport(2)");
		return 0;
	}

	if (read(miSistemaDeFicheros->discoVirtual, &buffer, bytesUltimos) == -1) {
		fprintf(stderr, "Falló read en myExport(2)");
		return 0;
	}

	if (write(handle, &buffer, bytesUltimos) == -1) {
		fprintf(stderr, "Falló write en myExport(2)");
		return 0;
	}
	...
	sync();
	printf("Fichero exportado\n");

	return 1;
}

void borraBloquesINodo(MiSistemaDeFicheros* miSistemaDeFicheros, EstructuraNodoI* Inodo) {
    uint i;
...
int myRm(MiSistemaDeFicheros* miSistemaDeFicheros, char* nombreArchivo) {
    ...
    /// Devuelve 1 si va bien, 0 en caso de fallo
    ...
    borraBloquesINodo(miSistemaDeFicheros, miSistemaDeFicheros->nodosI[numNodoI]);
