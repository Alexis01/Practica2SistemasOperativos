...
int estado(char*, int (*)(const char*, struct stat*));
...
int main(int argc, char *argv[]) {
	...
	statFunc = (argc > 2) && (strcmp(argv[1], "-L") == 0) ? lstat : stat;

	// Para cada fichero mostrar su estado
	for (i = statFunc == lstat ? 2 : 1; i<argc; i++)
		estado(argv[i], statFunc);

	exit(0);
}

int estado(char *fichero, int (*statFunc)(const char*, struct stat*)) {
	struct stat buf;
	struct passwd *pw;
	struct group *gr;
	int i;
